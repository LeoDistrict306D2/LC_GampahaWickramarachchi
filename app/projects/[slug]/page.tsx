import type { Metadata } from 'next';
import { notFound } from 'next/navigation';
import Link from 'next/link';
import { club } from '@/content/club';
import { projects } from '@/content/projects';
import { byDateDesc, formatDate } from '@/lib/utils';
import { PageMasthead } from '@/components/PageMasthead';
import { Photo } from '@/components/Photo';
import { accessionFor } from '@/components/SpecimenSheet';

export function generateStaticParams() {
  return projects.map((project) => ({ slug: project.slug }));
}

type Params = { params: Promise<{ slug: string }> };

export async function generateMetadata({ params }: Params): Promise<Metadata> {
  const { slug } = await params;
  const project = projects.find((entry) => entry.slug === slug);
  if (!project) return {};

  return {
    title: project.title,
    description: project.summary,
    alternates: { canonical: `/projects/${project.slug}` },
    openGraph: {
      type: 'article',
      title: `${project.title} — ${club.name}`,
      description: project.summary,
      publishedTime: project.date,
      images: [
        {
          url: project.heroImage.src,
          width: project.heroImage.width,
          height: project.heroImage.height,
          alt: project.heroImage.alt,
        },
      ],
    },
  };
}

export default async function ProjectPage({ params }: Params) {
  const { slug } = await params;
  const project = projects.find((entry) => entry.slug === slug);
  if (!project) notFound();

  const ordered = byDateDesc(projects);
  const accession = accessionFor(ordered.length, ordered.indexOf(project));

  const related = projects
    .filter((entry) => entry.slug !== project.slug)
    .sort((a, b) => {
      const aMatch = a.category === project.category ? 0 : 1;
      const bMatch = b.category === project.category ? 0 : 1;
      return aMatch - bMatch || b.date.localeCompare(a.date);
    })
    .slice(0, 3);

  return (
    <>
      <PageMasthead
        kicker={`${accession} · ${project.category.replace(/-/g, ' ')}`}
        title={project.title}
        standfirst={project.summary}
        breadcrumb={{ href: '/projects', label: 'All records' }}
      />

      <div className="wrap pt-10">
        <Photo image={project.heroImage} ratio="wide" priority sizes="100vw" plate="Pl. 1" />
      </div>

      <div className="wrap band grid gap-12 md:grid-cols-12">
        <div className="md:col-span-7">
          {project.story && project.story.length > 0 ? (
            project.story.map((paragraph, index) => (
              <p
                key={index}
                className="measure mb-6 text-lg leading-relaxed text-ink-muted last:mb-0"
              >
                {paragraph}
              </p>
            ))
          ) : (
            <p className="measure text-lg leading-relaxed text-ink-muted">{project.summary}</p>
          )}

          {project.objectives && project.objectives.length > 0 ? (
            <section className="mt-12" aria-labelledby="objectives">
              <p className="label" id="objectives">
                Aims
              </p>
              <ol className="mt-4 border-t border-rule">
                {project.objectives.map((objective, index) => (
                  <li key={objective} className="flex gap-5 border-b border-rule py-3.5">
                    <span className="label mt-1 shrink-0" aria-hidden>
                      {String(index + 1).padStart(2, '0')}
                    </span>
                    <span className="text-ink-muted">{objective}</span>
                  </li>
                ))}
              </ol>
            </section>
          ) : null}
        </div>

        <aside className="md:col-span-4 md:col-start-9">
          <div className="sheet">
            <p className="label">Determination</p>
            <dl className="determination mt-4 border-t border-rule">
              {[
                { term: 'Accession', value: accession },
                { term: 'Date', value: formatDate(project.date) },
                { term: 'Leo year', value: project.year },
                { term: 'Locality', value: project.location },
                { term: 'Category', value: project.category.replace(/-/g, ' ') },
              ]
                .filter((row) => Boolean(row.value))
                .map((row) => (
                  <div key={row.term} className="flex justify-between gap-4 border-b border-rule py-2.5">
                    <dt>{row.term}</dt>
                    <dd className="text-right capitalize">{row.value}</dd>
                  </div>
                ))}
            </dl>

            {project.impact && project.impact.length > 0 ? (
              <>
                <p className="label mt-8">Measurements</p>
                <dl className="mt-4 border-t border-rule">
                  {project.impact.map((stat) => (
                    <div
                      key={stat.id}
                      className="flex items-baseline justify-between gap-4 border-b border-rule py-3"
                    >
                      <dt className="label text-ink-faint">{stat.label}</dt>
                      <dd className="font-serif text-2xl tabular-nums text-ink">
                        {stat.prefix}
                        {typeof stat.value === 'number'
                          ? stat.value.toLocaleString('en-LK')
                          : stat.value}
                        {stat.suffix}
                      </dd>
                    </div>
                  ))}
                </dl>
              </>
            ) : null}

            {project.partners && project.partners.length > 0 ? (
              <>
                <p className="label mt-8">In collaboration with</p>
                <ul className="mt-3 space-y-1.5">
                  {project.partners.map((partner) => (
                    <li key={partner.name} className="text-sm text-ink-muted">
                      {partner.name}
                    </li>
                  ))}
                </ul>
              </>
            ) : null}
          </div>
        </aside>
      </div>

      {project.gallery && project.gallery.length > 0 ? (
        <section className="wrap pb-16" aria-labelledby="plates">
          <p className="label" id="plates">
            Plates
          </p>
          <div className="mt-5 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {project.gallery.map((image, index) => (
              <Photo
                key={image.src}
                image={image}
                ratio="plate"
                plate={`Pl. ${index + 2}`}
                sizes="(min-width: 1024px) 33vw, (min-width: 640px) 50vw, 100vw"
              />
            ))}
          </div>
        </section>
      ) : null}

      {related.length > 0 ? (
        <section className="border-t border-rule-strong bg-panel band" aria-labelledby="related">
          <div className="wrap">
            <p className="label" id="related">
              Related records
            </p>
            <ul className="mt-6 border-t border-rule">
              {related.map((entry) => (
                <li key={entry.id} className="border-b border-rule">
                  <Link
                    href={`/projects/${entry.slug}`}
                    className="flex flex-col gap-1 py-5 transition-colors hover:text-accent sm:flex-row sm:items-baseline sm:justify-between sm:gap-6"
                  >
                    <span className="font-serif text-xl">{entry.title}</span>
                    <span className="label shrink-0 text-ink-faint">
                      {formatDate(entry.date, { year: 'numeric', month: 'short' })}
                    </span>
                  </Link>
                </li>
              ))}
            </ul>
          </div>
        </section>
      ) : null}
    </>
  );
}
